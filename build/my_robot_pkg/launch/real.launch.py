...
def generate_launch_description():
    ...
    # 1. Nav2 실행 (Map Server, AMCL, Controller Server 등)
    ...
    # 3. [추가됨] 커스텀 A* Planner 노드 실행 (obstacle.py)
    # 주의: setup.py의 entry_points에 등록한 이름('obstacle_node')을 executable에 적어야 함
    start_planner_cmd = Node(
        package=pkg_name,
        executable='obstacle',  # setup.py에 등록된 이름
        name='a_star_mppi_planner',  # 노드 이름
        output='screen',
        parameters=[{'use_sim_time': use_sim_time}]
    )
    ...
    ld.add_action(start_planner_cmd)  # A* 노드 실행 추가
